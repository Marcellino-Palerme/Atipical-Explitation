...
echo "\#SBATCH --job-name=tl_atipical_B4_${my_date}" >> $temp_file
echo '#SBATCH --gres=gpu:1' >> $temp_file
echo '#SBATCH -p gpu' >> $temp_file
echo '#SBATCH --mem=50G' >> $temp_file
echo "\#SBATCH --output=./report/${my_date}_tl_atipical_B4/${my_date}_results_tl_atipical_B4.txt" >> $temp_file

echo . /softs/local/env/envconda.sh >> $temp_file
echo conda activate /home/genouest/inra_umr1349/mpalerme/conda >> $temp_file
echo "python tl_atipical_B4.py ${my_date}" >> $temp_file

#Create directory of report
mkdir -p "./report/${my_date}_tl_atipical_B4"
