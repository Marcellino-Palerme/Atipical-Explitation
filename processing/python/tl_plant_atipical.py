...
"""
PATH = "/home/genouest/inra_umr1349/mpalerme/Plant_leave_with_augmentation" 
...
top_dropout_rate = 0.2
model_bis.add(tf.keras.layers.Dropout(top_dropout_rate, name="top_dropout"))

model_bis.add(tf.keras.layers.Dense(n_classes, activation='softmax'))

base_learning_rate = 0.001
model_bis.compile(optimizer=tf.keras.optimizers.Adam(lr=base_learning_rate),
                  loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=False),
                  metrics=['accuracy'])

history = model_bis.fit(train_dataset,
                        epochs=30,
                        validation_data=validation_dataset,
                        verbose=2)
