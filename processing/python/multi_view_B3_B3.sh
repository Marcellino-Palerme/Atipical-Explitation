...
echo "#SBATCH --job-name=multi_view_${struc}_${struc}_${my_date}" >> $temp_file
echo '#SBATCH --gres=gpu:1' >> $temp_file
echo '#SBATCH -p gpu' >> $temp_file
echo '#SBATCH --mem=50G' >> $temp_file
echo "#SBATCH --output=./report/${my_date}_multi_view_${struc}_${struc}/${my_date}_results_tl_atipical_${struc}_${struc}.txt" >> $temp_file

echo . /softs/local/env/envconda.sh >> $temp_file
echo conda activate /home/genouest/inra_umr1349/mpalerme/conda >> $temp_file
echo "python multi_view.py ${my_date} ${struc}" >> $temp_file

#Create directory of report
mkdir -p "./report/${my_date}_multi_view_${struc}_${struc}"
