...
do
   ...
   do
      ...
      then
         ...
         ln -s "$1/$class/${lt_images[$index]}" "$2/train/$class"
      elif (( $(echo "$index < $valid_index" | bc -l) )) 
      then
         # add symbolic link of image in validation
         ln -s "$1/$class/${lt_images[$index]}" "$2/validation/$class"
      elif (( $(echo "$index >= $valid_index" | bc -l) ))
      then
         # add symbolic link of image in test
         ln -s "$1/$class/${lt_images[$index]}" "$2/test/$class"
      fi
   done
